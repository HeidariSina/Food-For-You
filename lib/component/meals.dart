...
class _MyMealsPageState extends State<MyMealsPage> {
  ...
  List<Meals> _selectedMeals = myMeals;
  TextEditingController _textInputControl = TextEditingController();

  void changeSelectedIndex(int index) {
    setState(() {
      if (index == 0) {
        _selectedMeals = myMeals;
      } else {
        _selectedMeals = myMeals.where(
          (element) {
                  ...
                  itemBuilder: (contex, index) {
                    if (_selectedindex == index) {
                      return MyCategoryCard(
                          category[index], true, index, changeSelectedIndex);
                    } else {
                      return MyCategoryCard(
                          category[index], false, index, changeSelectedIndex);
                    }
                  },
